...
手冊依目前 M8 的實作撰寫。程式與文件檢查已通過，但 Discord 權限、角色階層與完整互動流程仍應先在測試 guild 實機驗收，再放進正式社群。尤其是 honeypot 的 ban，別拿正式成員試，會很有記憶點，但不是好的那種。
...
如果角色位置太高，Bot 會顯示警告，但仍會保存設定。這時要回 Discord 的身分組設定調整順序。
...
- 分數 **大於或等於 high**：尚未持有角色的成員會取得角色。
- 分數 **小於 low**：已持有角色的成員會失去角色。
- 分數落在兩者中間：維持目前狀態。

`high` 必須大於或等於 `low`。分開兩條線是為了避免成員分數剛好卡在邊界時，角色每天被加上又拔掉。
...
只要成員持有任何一個豁免角色，排程就不會變更他的活躍成員角色。
...
Bot、持有豁免角色的人，以及 Bot 無法管理的角色都會被跳過。若排程完全沒有動作，先檢查：

1. `/config view` 是否已設定角色和門檻。
...
- `title`：顯示在訊息上方的標題
- `channel`：文字或公告頻道
- `role_1` 與 `label_1`：第一個角色與按鈕文字

最多可以設定 5 組角色。從第二組開始可省略，但 `role_n` 與 `label_n` 必須一起填。
...
成員按一次會取得角色，再按一次會移除角色。回覆只有本人看得到。
...
- 刪除訊息不會同步刪除資料庫紀錄。
- Bot 需要 `Manage Roles`，角色必須在 Bot 最高身分組下方，也不能是其他 integration 管理的角色。
...
Honeypot 會對在指定頻道發言的非工作人員採取動作。正式啟用前，先確認管理紀錄頻道、Bot 權限與角色階層，並用測試帳號驗收。
...
伺服器擁有者與持有 `Manage Messages` 的成員視為工作人員，不會被處置。其他管理角色若沒有 `Manage Messages`，不會因為名稱看起來像管理員就自動豁免。
...
| 變數                               | 預設值        | 用途                           |
| ---------------------------------- | ------------- | ------------------------------ |
| `ACTIVITY_TIME_ZONE`               | `Asia/Taipei` | 每日分界與活躍角色排程時區     |
...
| `ACTIVE_MEMBER_CRON`               | `0 4 * * *`   | 活躍角色更新排程               |
| `EMBED_DRAFT_TTL_MINUTES`          | `60`          | Embed 草稿存活時間             |
| `HONEYPOT_TIMEOUT_SECONDS`         | `600`         | Honeypot 預設 timeout 秒數     |

修改 `.env` 後需要重新啟動 Bot。排程值若不是有效 cron expression，Bot 仍可上線，但活躍角色 sweep 不會被排程，log 會留下錯誤。
...
確認 Bot 具有 Manage Messages，以及對應的 Moderate Members 或 Ban Members。若成員角色高於 Bot、成員是伺服器擁有者，或持有 Manage Messages，Bot 不會處置。
