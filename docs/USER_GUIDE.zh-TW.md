...
| 讓成員建立臨時語音房   | `/config dynamic-voice`                        |
...
| 動態語音頻道          | Manage Channels、Manage Roles、Move Members |
...
先建立一個讓成員加入的入口語音頻道，例如 `➕ 建立語音頻道`，再執行：

```text
/config dynamic-voice enabled:True channel:➕ 建立語音頻道
```

成員加入入口後，Bot 會在同一個分類建立 `成員顯示名稱 的語音頻道`，把成員移進去，並授予該成員管理此頻道名稱與權限覆寫所需的權限。最後一位成員離開後，Bot 會刪除頻道。

Bot 會把自己建立的頻道 ID 與擁有者寫進 PostgreSQL。這不是拿來保存語音內容，而是讓 Bot 重啟後仍能辨認哪些空頻道可以安全清掉。若建立頻道、保存紀錄或移動成員其中一步失敗，Bot 會嘗試回滾剛建立的空頻道並留下錯誤 log。

停用時不必重選入口：

```text
/config dynamic-voice enabled:False
```

停用只會阻止之後從入口建立新頻道；已存在的動態頻道仍會在變空時正常清理。
...
確認入口仍是 `/config view` 顯示的頻道，並檢查 Bot 在入口及其分類是否具有 Manage Channels、Manage Roles、Move Members、View Channel 與 Connect。若成員成功進入入口但沒有被移動，查看 log 裡的 `dynamic voice creation failed`；若空頻道留著，查看 `dynamic voice cleanup failed` 或 `dynamic voice startup reconciliation failed`。
...
- [ ] `/ping` 正常回覆。
- [ ] `/config view` 顯示預期設定。
- [ ] 加入動態語音入口後會建立專屬頻道、可修改名稱與權限，離開最後一人後會自動刪除。
- [ ] 歡迎、離開與管理紀錄頻道都能收到訊息。
